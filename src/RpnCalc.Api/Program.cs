...
using RpnCalc.Api.RateLimiting;
...
app.UseMiddleware<EvaluateRateLimiterMiddleware>();
app.UseCors("spa");
...
static Task<IResult> HandleAsync(Func<IResult> action)
{
    ...
    {
        return Task.FromResult(ToProblem(ex));
    }
}
...
static IResult ToProblem(Exception exception)
{
    ...
    {
        CalculatorDomainException domain => CreateProblem("Domain violation", domain.Message, StatusCodes.Status400BadRequest),
        DivideByZeroException divide => CreateProblem("Divide by zero", divide.Message, StatusCodes.Status400BadRequest),
        _ => CreateProblem("Unexpected error", exception.Message, StatusCodes.Status500InternalServerError)
    };
}

static IResult CreateProblem(string title, string detail, int statusCode)
{
